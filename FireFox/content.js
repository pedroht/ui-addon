...
// Global variables
  ...
  function selectMonsterCard(index) {
    // Only select from joinable AND visible monster cards (those with active join buttons and not filtered out)
    const allMonsterCards = document.querySelectorAll('.monster-card');
    const joinableCards = [];

    allMonsterCards.forEach(card => {
      // Skip hidden/filtered cards
    ...
    if (index >= 0 && index < joinableCards.length) {
      const card = joinableCards[index];
      const joinBtn = card.querySelector('.join-btn, button[onclick*="join"], a[href*="battle.php"]');
  ...
  function addMonsterCardHotkeyOverlays() {
    ...
    const allMonsterCards = document.querySelectorAll('.monster-card');
    let overlayIndex = 0; // Track which hotkey number to assign

    allMonsterCards.forEach((card) => {
      // Skip if we've already assigned 9 hotkeys
      if (overlayIndex >= 9) return;

      // Check if overlay already exists
      if (card.querySelector('.hotkey-overlay')) return;
      ...
      // Only add overlay if monster is joinable
      if (isJoinable) {
        const overlay = document.createElement('div');
        overlay.className = 'hotkey-overlay';
        overlay.textContent = extensionSettings.hotkeys.monsterSelectionKeys[overlayIndex]?.toUpperCase() || (overlayIndex + 1).toString();
        overlay.style.cssText = `
          position: absolute;
          top: 5px;
          right: 5px;
        ...
        overlayIndex++; // Increment for next joinable monster
      }
    });
  }
